...
@section('breadcrumb')
    ...
    <li class="active"><a class="text-muted">标准颜色</a></li>
@stop
...
@section('tableRows')
    @foreach ($rows as $row)
        <tr>
            ...
            <td id="sColorName{{ $row->id }}">{{ $row->name }}</td>
            <td id="sColorPinyin{{ $row->id }}">{{ $row->pinyin }}</td>
            <td id="sColorMaterialName{{$row->id}}">{{$row->material_name}}</td>
            <td id="sColorLetter{{ $row->id }}">{{ $row->letter }}</td>
            <input type="hidden" id="sColorMaterialID{{$row->id}}" value="{{$row->material_id}}"/>
            <td>
                ...
                <a class="btn btn-xs btn-danger m-b-none" type="button"
                   href="/console/scolor/destroy/{{ $row->id }}">删除</a>
            </td>
        </tr>
    @endforeach
@stop
