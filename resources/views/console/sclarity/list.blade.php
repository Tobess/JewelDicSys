...
@section('breadcrumb')
    ...
    <li class="active"><a class="text-muted">标准净度</a></li>
@stop
...
@section('tableRows')
    @foreach ($rows as $row)
        <tr>
            ...
            <td id="sClarityName{{ $row->id }}">{{ $row->name }}</td>
            <td id="sClarityPinyin{{ $row->id }}">{{ $row->pinyin }}</td>
            <td id="sClarityMaterialName{{$row->id}}">{{$row->material_name}}</td>
            <td id="sClarityLetter{{ $row->id }}">{{ $row->letter }}</td>
            <input type="hidden" id="sClarityMaterialID{{$row->id}}" value="{{$row->material_id}}"/>
            <td>
                ...
                <a class="btn btn-xs btn-danger m-b-none" type="button"
                   href="/console/sclarity/destroy/{{ $row->id }}">删除</a>
            </td>
        </tr>
    @endforeach
@stop
...
@section('scripts')
    <script>
        function save(id) {
            ...
            mWin.find('form').attr('action', "/console/sclarity/" + (id > 0 ? ('update/' + id) : 'store'));
            if (id > 0) {
                mWin.find('[name="name"]').val($("#sClarityName" + id).text());
                mWin.find('[name="pinyin"]').val($("#sClarityPinyin" + id).text());
                mWin.find('[name="letter"]').val($("#sClarityLetter" + id).text());
                mWin.find('[name="material"]').val($("#sClarityMaterialID" + id).val());
                mWin.find('[name="material-show"]').val($("#sClarityMaterialName" + id).text());
            }
