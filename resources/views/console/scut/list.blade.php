...
@section('breadcrumb')
    ...
    <li class="active"><a class="text-muted">标准切工</a></li>
@stop
...
@section('tableRows')
    @foreach ($rows as $row)
        <tr>
            ...
            <td id="sCutName{{ $row->id }}">{{ $row->name }}</td>
            <td id="sCutPinyin{{ $row->id }}">{{ $row->pinyin }}</td>
            <td id="sCutMaterialName{{$row->id}}">{{$row->material_name}}</td>
            <td id="sCutLetter{{ $row->id }}">{{ $row->letter }}</td>
            <input type="hidden" id="sCutMaterialID{{$row->id}}" value="{{$row->material_id}}"/>
            <td>
                ...
                <a class="btn btn-xs btn-danger m-b-none" type="button"
                   href="/console/scut/destroy/{{ $row->id }}">删除</a>
            </td>
        </tr>
    @endforeach
@stop
...
@section('scripts')
    <script>
        function save(id) {
            ...
            mWin.find('form').attr('action', "/console/scut/" + (id > 0 ? ('update/' + id) : 'store'));
            if (id > 0) {
                mWin.find('[name="name"]').val($("#sCutName" + id).text());
                mWin.find('[name="pinyin"]').val($("#sCutPinyin" + id).text());
                mWin.find('[name="letter"]').val($("#sCutLetter" + id).text());
                mWin.find('[name="material"]').val($("#sCutMaterialID" + id).val());
                mWin.find('[name="material-show"]').val($("#sCutMaterialName" + id).text());
            }
