...
@section('breadcrumb')
    ...
    <li class="active"><a class="text-muted">标准形状</a></li>
@stop
...
@section('tableRows')
    @foreach ($rows as $row)
        <tr>
            ...
            <td id="sShapeName{{ $row->id }}">{{ $row->name }}</td>
            <td id="sShapePinyin{{ $row->id }}">{{ $row->pinyin }}</td>
            <td id="sShapeMaterialName{{$row->id}}">{{$row->material_name}}</td>
            <td id="sShapeLetter{{ $row->id }}">{{ $row->letter }}</td>
            <input type="hidden" id="sShapeMaterialID{{$row->id}}" value="{{$row->material_id}}"/>
            <td>
                ...
                <a class="btn btn-xs btn-danger m-b-none" type="button"
                   href="/console/sshape/destroy/{{ $row->id }}">删除</a>
            </td>
        </tr>
    @endforeach
@stop
...
@section('scripts')
    <script>
        function save(id) {
            ...
            mWin.find('form').attr('action', "/console/sshape/" + (id > 0 ? ('update/' + id) : 'store'));
            if (id > 0) {
                mWin.find('[name="name"]').val($("#sColorName" + id).text());
                mWin.find('[name="pinyin"]').val($("#sColorPinyin" + id).text());
                mWin.find('[name="letter"]').val($("#sColorLetter" + id).text());
                mWin.find('[name="material"]').val($("#sColorMaterialID" + id).val());
                mWin.find('[name="material-show"]').val($("#sColorMaterialName" + id).text());
            }
