...
@section('breadcrumb')
    ...
    <li class="active"><a class="text-muted">珠宝款式</a></li>
@stop
...
<div class="input-group">
    <input id="searchQueryBox" type="text" class="input-sm form-control" placeholder="请输入款式查询" value="{{ isset($query) && $query ? $query : ''}}">
        <span class="input-group-btn">
...
@section('tableRows')
    ...
    <tr>
        ...
        <td id="stylePinyin{{ $row->id }}">{{ $row->pinyin }}</td>
        <td id="styleLetter{{ $row->id }}">{{ $row->letter }}</td>
        <td>
...
<script>
    function save(id) {
        ...
        if (id > 0) {
            ...
            mWin.find('[name="pinyin"]').val($("#stylePinyin"+id).text());
            mWin.find('[name="letter"]').val($("#styleLetter"+id).text());
        }
