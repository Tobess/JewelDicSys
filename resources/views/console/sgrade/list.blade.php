...
@section('breadcrumb')
    ...
    <li class="active"><a class="text-muted">标准等级</a></li>
@stop
...
@section('tableRows')
    @foreach ($rows as $row)
        <tr>
            ...
            <td id="sGradeName{{ $row->id }}">{{ $row->name }}</td>
            <td id="sGradePinyin{{ $row->id }}">{{ $row->pinyin }}</td>
            <td id="sGradeMaterialName{{$row->id}}">{{$row->material_name}}</td>
            <td id="sGradeLetter{{ $row->id }}">{{ $row->letter }}</td>
            <input type="hidden" id="sGradeMaterialID{{$row->id}}" value="{{$row->material_id}}"/>
            <td>
                ...
                <a class="btn btn-xs btn-danger m-b-none" type="button"
                   href="/console/sgrade/destroy/{{ $row->id }}">删除</a>
            </td>
        </tr>
    @endforeach
@stop
...
@section('scripts')
    <script>
        function save(id) {
            ...
            mWin.find('form').attr('action', "/console/sgrade/" + (id > 0 ? ('update/' + id) : 'store'));
            if (id > 0) {
                mWin.find('[name="name"]').val($("#sGradeName" + id).text());
                mWin.find('[name="pinyin"]').val($("#sGradePinyin" + id).text());
                mWin.find('[name="letter"]').val($("#sGradeLetter" + id).text());
                mWin.find('[name="material"]').val($("#sGradeMaterialID" + id).val());
                mWin.find('[name="material-show"]').val($("#sGradeMaterialName" + id).text());
            }
